                ...
                <div className="mt-4 row mb-5" style={{minheight:"100%",  background:'black !important'}}>
                  <div className="col-lg h-75">
                    <div className="d-flex justify-content-center  h-100">
                      ...
                      This project allows users to control the color of my custom LED tubes via Twitch chat, YouTube chat and a website. The website front-end was made using ReactJS in combination with Bootstrap v5 for responsive CSS and uses an ExpressJS backend. A user's color choice on the website is sent over a SocketIO instance to an external listener component on my desktop made using NodeJS. Another external listener made in NodeJS uses Twitch’s API to get real time chat data, and a Python component using Selenium retrieves YouTube chat data. That data is sent internally to a Redis queue, and then published a channel. A database listener, Arduino listener and WebSocket server listener subscribe to that channel. The database listener adds the colors that were chosen to a MongoDB database. The Arduino listener writes the color data to the Arduino via Serial, from there the Arduino sends a PWM signal to control the led tubes via a semi-custom perf board circuit (initially based off a Adafruit guide).  The WebSocket server listener sends that data it receives websocket clients. Another program Control My Lights Video Dashboard connects to that server and runs an animation when it receives new color data.                       </p>
                    </div>
                  </div>
                  <div className="col-lg h-75 ">
                        <ul id="githubinks" >
